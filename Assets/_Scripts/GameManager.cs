...
public class GameManager : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (Input.GetMouseButtonDown(0))
        {
            ...
            if (Physics.Raycast(ray, out var hit, Mathf.Infinity, dragObjectsLayer))
            {
        ...
        if (Input.GetMouseButton(0) && holding && selectedObject != null) 
        {
            ...
            if (Physics.Raycast(ray, out var hit, Mathf.Infinity, groundsLayer))
            {
                pos1 = hit.point;
        ...
        if (!Input.GetMouseButtonUp(0) || !holding || selectedObject == null) return;
        {
            var dropPosition = selectedObject.transform.position;
    ...
    {
        if (FinanceManager.Instance.totalCoin < LevelSpecial.Instance.fabricCost || sewingAreaLayoutOrganizer.transform.childCount > 4) return;
